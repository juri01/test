...
public class PaymentFacade {
    ...
    public Mono<PaymentAuthResponse> authenticate(PaymentAuthRequest request) {
        ...
        return validateOrder(request.orderId())
                .then(checkDuplicatePayment(request.orderId()))
                .then(provider.authenticate(request))
                .flatMap(response -> savePaymentHistory(request, response)
                        .thenReturn(response));
    }
    ...
    public Mono<PaymentApproveResponse> approve(PaymentApproveRequest request) {
        ...
        return findTransaction(request.transactionId())
                .then(provider.approve(request))
                .flatMap(response -> updatePaymentStatus(response)
                        .thenReturn(response));
    }
    ...
    public Mono<PaymentCancelResponse> cancel(PaymentCancelRequest request) {
        ...
        return findTransaction(request.transactionId())
                .then(validateCancelable(request.transactionId(), request.cancelAmount()))
                .then(provider.cancel(request))
                .flatMap(response -> updateCancelStatus(response)
                        .thenReturn(response));
    }

    // ── 내부 로직 (DB 조회, 검증) ────────────────────────────────

    private Mono<Void> validateOrder(String orderId) {
        // TODO: 주문 존재 여부 DB 조회
        log.debug("주문 유효성 검증 - orderId: {}", orderId);
        return Mono.empty();
    }

    private Mono<Void> checkDuplicatePayment(String orderId) {
        // TODO: 중복 결제 여부 DB 조회
        log.debug("중복 결제 체크 - orderId: {}", orderId);
        return Mono.empty();
    }

    private Mono<Void> findTransaction(String transactionId) {
        // TODO: 거래 존재 여부 DB 조회
        log.debug("거래 조회 - transactionId: {}", transactionId);
        return Mono.empty();
    }

    private Mono<Void> validateCancelable(String transactionId, java.math.BigDecimal cancelAmount) {
        // TODO: 취소 가능 여부 검증 (이미 취소된 건인지, 금액 초과인지 등)
        log.debug("취소 가능 여부 검증 - transactionId: {}, cancelAmount: {}", transactionId, cancelAmount);
        return Mono.empty();
    }

    private Mono<Void> savePaymentHistory(PaymentAuthRequest request, PaymentAuthResponse response) {
        // TODO: 결제 인증 이력 DB 저장
        log.debug("결제 이력 저장 - orderId: {}, transactionId: {}", request.orderId(), response.transactionId());
        return Mono.empty();
    }

    private Mono<Void> updatePaymentStatus(PaymentApproveResponse response) {
        // TODO: 결제 승인 상태 DB 업데이트
        log.debug("결제 상태 업데이트 - transactionId: {}", response.transactionId());
        return Mono.empty();
    }

    private Mono<Void> updateCancelStatus(PaymentCancelResponse response) {
        // TODO: 취소 상태 DB 업데이트
